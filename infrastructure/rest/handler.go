...
func (h *handler) GenerateFeed(w http.ResponseWriter, r *http.Request) {
	...
	go h.generateFeed(generationType)
	w.WriteHeader(http.StatusCreated)
}

func (h *handler) generateFeed(generationType string) {
	err := h.feeds.GenerateFeed(context.Background(), generationType)
	if err != nil {
		log.Error().Err(err).Str("generation_type", generationType).Msg("feeds.GenerateFeed")
	}
}
...
func (h *handler) ListSchedules(w http.ResponseWriter, r *http.Request) {
	...
	jsonResponse(w, http.StatusOK, schedulesOut)
}
